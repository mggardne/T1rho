...
sdat = datetime(zeros(nseries,1),0,0); % Series date string
%
for k = 1:nseries
%
   l = id(k);
...
colnams = {'Series#','SeriesDateTime','SeriesDescription', ...
           ...
           'Columns','PixelX','PixelY','SpacingBetweenSlices', ...
           'SliceThickness','ImageRows','ImageColumns','Width', ...
           'Height','ImagePixelX','ImagePixelY','RescaleSlope', ...
           'RescaleIntercept'};
t0 = table(sn,sdat,string(stxt),string(ptxt),string(im_type(idx)), ...
           ...
           sspc,sthk,psz(:,1),psz(:,2),isz(:,1),isz(:,2), ...
